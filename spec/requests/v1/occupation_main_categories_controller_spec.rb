...
RSpec.describe V1::OccupationMainCategoriesController, type: :request do
  let(:account) { create(:account) }
  let(:headers) { { Authorization: "Bearer #{account.jwt}" } }

  describe 'GET /v1/occupation_main_categories' do
    let(:request) { get v1_occupation_main_categories_path, headers: headers }
